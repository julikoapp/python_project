...
data_func = Database("bookstore.db") #sorry too lazy to replace ;)
...
class Window:
    ...
    def get_selected_row(self,event):
        try:
            index = self.list_1.curselection()[0]  # curselection - returns tuple цифры row на которую нажали
            self.selected_tuple = self.list_1.get(index)  # gives row as a tuple
            # чтобы выделенный курсором  строка передела значения в места наверху
    ...
    def search_command(self):
        self.list_1.delete(0, END)
        for row in data_func.find_book(self.title_text.get(), self.author_name.get(), self.year_tx.get(), self.isbn_tx.get()):
            self.list_1.insert(END, row)
    ...
    def update_command(self):
        data_func.update(self.selected_tuple[0], self.title_text.get(), self.author_name.get(), self.year_tx.get(), self.isbn_tx.get())
...
#     try:
#         index = list_1.curselection()[0] #curselection - returns tuple цифры row на которую нажали
#         selected_tuple=list_1.get(index) # gives row as a tuple
#         # чтобы выделенный курсором  строка передела значения в места наверху
#         title.delete(0,END)
...
# def search_command():
#     list_1.delete(0, END)
#     for row in data_func.find_book(title_text.get(), author_name.get(), year_tx.get(), isbn_tx.get()):
#         list_1.insert(END, row)
#
# def add_command():
#     list_1.delete(0, END)
#     data_func.add_book(title_text.get(), author_name.get(), year_tx.get(), isbn_tx.get())
#     view_command()
#
# def delete_command():
#     data_func.delete_book(selected_tuple[0])
#
# def update_command():
#     data_func.update(selected_tuple[0],title_text.get(), author_name.get(), year_tx.get(), isbn_tx.get())
...
# author = Entry(window,textvariable=author_name)
# author.grid(row=0,column=3)
#
# year_tx = StringVar()
# year = Entry(window,textvariable=year_tx)
# year.grid(row=1,column=1)
#
# isbn_tx = StringVar()
# isbn = Entry(window,textvariable=isbn_tx)
